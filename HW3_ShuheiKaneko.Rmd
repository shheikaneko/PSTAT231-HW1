...
### Question 10
```{r, warning=FALSE}
log_roc_test <- augment(log_fit, new_data = titanic_test) %>%
  roc_curve(survived, .pred_No)
log_roc_test <- log_roc_test %>% mutate(model = "Logistic")

lda_roc_test <- augment(lda_fit, new_data = titanic_test) %>%
  roc_curve(survived, .pred_No)
lda_roc_test <- lda_roc_test %>% mutate(model = "LDA")

qda_roc_test <- augment(qda_fit, new_data = titanic_test) %>%
  roc_curve(survived, .pred_No)
qda_roc_test <- qda_roc_test %>% mutate(model = "QDA")

nb_roc_test <- augment(nb_fit, new_data = titanic_test) %>%
  roc_curve(survived, .pred_No)
nb_roc_test <- nb_roc_test %>% mutate(model = "Naive Bayes")

output_dataset <- log_roc_test %>%
  bind_rows(lda_roc_test) %>%
  bind_rows(qda_roc_test) %>%
  bind_rows(nb_roc_test)
